...
The command the agent actually executed (`tool_input.command` of the PostToolUse input). Differs from [`command`](#command-1) when the PreToolUse response rewrote the command via `updatedInput` — with runok that happens for [sandbox wrapping](/getting-started/claude-code/#sandbox-execution), producing the `runok exec --hook-origin <token> --sandbox <preset> -- '<command>'` form.
