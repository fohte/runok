...
Represents an evaluation result. The `type` field is a discriminator; `detail` is omitted for `allow` and `passthrough`, and present (with type-specific keys) for `deny` and `ask`.
...
When no rule matches, the configured [`default_action`](#default_action) is applied directly: `type` is `"allow"`, `"deny"`, `"ask"`, or `"passthrough"` accordingly. There is no separate `"default"` discriminator in the audit-log JSON.

### `type`

The kind of action.

**Type:** `"allow" | "deny" | "ask" | "passthrough"`\
**Always present:** Yes
...
| `passthrough` | No decision was made; the caller's own permission flow decided instead. |
