...
- **`PreToolUse`** (or absent, for hand-crafted/minimal input): evaluate the command against the rules and print the [`permissionDecision` response](#pretooluse-response) to stdout. When the command resolves to [`defaults.action: passthrough`](/configuration/schema/#defaultsaction), nothing is written to stdout and the exit code is `0` -- Claude Code interprets this as "no decision to report" and falls through to its own permission flow (allow/ask/deny rules, then read-only auto-approval, then the auto-mode classifier) instead of runok forcing an `ask` prompt.
