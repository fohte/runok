...
- **`argv[0]` contains a `/`.** A relative or absolute path (`./script.sh`, `/usr/local/bin/tool`) is skipped, since resolving it correctly would require the sandboxed process's working directory, which can differ from the hook process evaluating the command.
