...
:::caution
`defaults.sandbox` cannot be combined with [`defaults.action: passthrough`](/configuration/schema/#defaultsaction). A passthrough decision produces no `updatedInput`, so the sandbox wrapping would be silently dropped -- this is rejected at config validation time.
:::
