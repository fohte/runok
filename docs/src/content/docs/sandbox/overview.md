...
The Claude Code hook applies a sandbox to a compound command like `cmd1 && cmd2` sub-command by sub-command: each sub-command that matches a sandboxed rule runs under exactly the preset its own rule named -- runok replaces that sub-command's own text (its `KEY=VALUE` env-assignment prefix and its own redirects included) with `runok exec --hook-origin <token> --sandbox <preset> -- '<sub-command>'`, shell-quoted, and returns the rewritten string via `updatedInput`. Only the pipe, `&&`, `||`, and `;` that join the sub-commands together are left outside every sandbox. A sub-command whose rule names no sandbox runs unsandboxed, even next to one that does.
...
If every sub-command that specifies a sandbox names the **same** preset (after deduplication), that preset is applied directly instead of a merged policy -- the same way a non-compound command's preset is applied, including under `defaults.action: pass`. Merging across two or more distinct presets is represented as repeated `--sandbox` flags instead: the hook writes `updatedInput` as `runok exec --hook-origin <token> --sandbox <preset-a> --sandbox <preset-b> -- '<command>'`, and `runok exec` resolves and merges the named presets itself. This applies for `allow`, `ask`, and `pass` alike -- a `pass` resolution is no longer escalated to `ask` just because two or more distinct presets are involved.
