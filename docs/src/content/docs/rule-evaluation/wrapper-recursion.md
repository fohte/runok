...
The depth counter increments each time a wrapped command is extracted and evaluated. The limit is defined as `MAX_WRAPPER_DEPTH = 10` in `src/rules/rule_engine/simple_eval.rs`.
