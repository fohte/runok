...
5. **Return**: The final action (`allow`, `deny`, `ask`, or `passthrough`) and sandbox policy. If no rule matched, the action is resolved to the configured [`defaults.action`](/configuration/schema/#defaultsaction) (defaulting to `ask`).
