...
| `command.cwd`       | `string` | Effective cwd at the moment the command runs: `EvalContext.cwd` joined with the cumulative effect of statically-resolvable `cd` invocations earlier in the same `&& / \|\| / ;` chain, best-effort canonicalized. The result is absolute and canonical only when `EvalContext.cwd` is absolute and `canonicalize` succeeds — otherwise the un-canonicalized join is returned. Falls back to the session cwd when a dynamic `cd` sinks the chain. |
