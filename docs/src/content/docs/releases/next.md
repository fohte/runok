...
`runok hook` now accepts `--agent codex` alongside the existing `--agent claude-code`, dispatching Codex CLI's `PreToolUse` and `PermissionRequest` hook events. Codex rejects `updatedInput` unless it is paired with an explicit `permissionDecision: "allow"`, so `allow` routes through `runok exec` and `pass` emits nothing. `PermissionRequest` has no `updatedInput` support at all, so it always reports a plain `{ behavior: "allow" }` or `{ behavior: "deny", message }`, ignoring sandbox presets entirely.

`ask` is handled differently by the two events: `PreToolUse` routes it through `runok exec --ask`, whose Codex exec policy enters the approval flow before the wrapper executes the command. `PermissionRequest` is unchanged -- an `ask` decision writes nothing, deferring to Codex's own approval flow. With the default `approvals_reviewer = "user"`, this reaches a human approval dialog; with `approvals_reviewer = "auto_review"`, Guardian decides `allow` or `deny` without a human dialog. The runok configuration must keep `runok exec --ask *` at `ask` when a broad `runok exec *` allow rule exists, or the rewritten command can be allowed by the hook before Codex handles the approval. The allow and prompt policy rules installed by [`runok init --scope user`](/cli/init/#what-the-wizard-does) are required for this routing. Because the allow rule is trusted by Codex, a command without a runok sandbox preset also bypasses Codex's workspace sandbox; see [Codex execution path](/sandbox/security-model/#codex-execution-path). See [`runok hook`](/cli/hook/#codex---agent-codex) for the full decision mapping.
...
Codex does not evaluate exec policies for shell commands prefixed with an environment-variable assignment, so the hook-generated wrapper could bypass the `runok exec` and `runok exec --ask` rules. Hook wrappers now pass the hook-origin marker as the hidden `--hook-origin <token>` flag while keeping `--ask` immediately after `exec`, so `ask` decisions reach Codex's approval flow:
