...
`ask` is handled differently by the two events, since Codex's `PreToolUse` hook has no way to open an approval prompt mid-call: it always reports `permissionDecision: "deny"`, with the reason text explaining that human judgment is needed (not a hard rejection) and that the model should ask the user in conversation and retry if approved. `PermissionRequest` is unchanged -- an `ask` decision writes nothing, deferring to Codex's own approval UI. See [`runok hook`](/cli/hook/#codex---agent-codex) for the full decision mapping.
