...
`runok exec --ask` executes commands whose runok rule result is `ask`, while `deny` remains blocked. `runok init --scope user` installs the Codex exec policy rules that allow `runok exec` and prompt for `runok exec --ask`, preparing Codex to request approval before executing the wrapper.
