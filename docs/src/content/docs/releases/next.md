...
`runok hook --agent codex` now routes `ask` decisions through `runok exec --ask`, so Codex can request human approval before executing the command. `allow` decisions also route through `runok exec` without requiring a sandbox preset, while `deny` and `pass` keep their existing behavior.
