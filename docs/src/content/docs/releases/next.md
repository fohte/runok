...
`ask` is handled differently by the two events: `PreToolUse` routes it through `runok exec --ask`, whose Codex exec policy opens the approval UI before the wrapper executes the command. `PermissionRequest` is unchanged -- an `ask` decision writes nothing, deferring to Codex's own approval UI. The allow and prompt policy rules installed by [`runok init --scope user`](/cli/init/#what-the-wizard-does) are required for this routing. Because the allow rule is trusted by Codex, a command without a runok sandbox preset also bypasses Codex's workspace sandbox; see [Codex execution path](/sandbox/security-model/#codex-execution-path). See [`runok hook`](/cli/hook/#codex---agent-codex) for the full decision mapping.
