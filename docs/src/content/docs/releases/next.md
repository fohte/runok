...
This marker is a plain CLI flag, not a cryptographic proof that a given invocation actually came from the hook -- anyone who can run `runok exec` at all can set it. This is a deliberate, accepted trade-off, not an oversight: it only relaxes `Action::Pass` (a command that matched no rule). A command that matches a `deny` or `ask` rule is still rejected by `exec`'s own re-evaluation regardless of the marker. Spoofing it can therefore only turn "the caller's own permission flow decides, unsandboxed" into "the caller's own permission flow decides, sandboxed" -- a strictly narrower outcome, not a privilege escalation.
