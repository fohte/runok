...
mod tests {
    ...
    fn audit_entry_roundtrip() {
        ...
        assert_eq!(deserialized.matched_rules, entry.matched_rules);
        assert_eq!(deserialized.sandbox_preset, entry.sandbox_preset);
        assert_eq!(deserialized.default_action, entry.default_action);
        assert_eq!(
            deserialized.metadata.endpoint_type,
            entry.metadata.endpoint_type
        );
        assert_eq!(deserialized.metadata.session_id, entry.metadata.session_id);
        assert_eq!(deserialized.metadata.cwd, entry.metadata.cwd);
        assert_eq!(deserialized.metadata.tool_name, entry.metadata.tool_name);
        assert_eq!(
            deserialized.metadata.hook_event_name,
            entry.metadata.hook_event_name
        );
        assert!(deserialized.sub_evaluations.is_none());
    }

    #[test]
    fn audit_entry_with_sub_evaluations_roundtrip() {
        let entry = AuditEntry {
        ...
        let sub_evals = deserialized.sub_evaluations.unwrap();
        assert_eq!(sub_evals.len(), 2);
        assert_eq!(sub_evals[0].command, "echo hello");
        assert_eq!(sub_evals[0].action, SerializableAction::Allow);
        assert_eq!(sub_evals[1].command, "rm -rf /");
    }
