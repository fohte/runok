...
mod tests {
    ...
    /// tokens. Mirrors the production helper in
    /// `rule_engine::flag_schema::build_flag_schema` so unit tests using
    /// `<flag:name>` see the same value-flag set the real command parser does.
