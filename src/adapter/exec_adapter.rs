...
/// denied commands produce stderr output and exit code 3. Ask decisions are
/// executed only when the internal `--ask` flag is present.
pub struct ExecAdapter {
    args: Vec<String>,
    sandbox_presets: Vec<String>,
    ask: bool,
    /// Set when this invocation was generated by a hook adapter's wrapper
    /// helper (via the hidden `--hook-origin <token>` flag), rather than typed
    /// by a user. Only in that case does `Action::Pass` defer to the sandbox
    /// instead of being treated as deny -- the hook already decided to route
    /// the permission decision through the agent's own flow.
    ///
    /// The token changes on every hook invocation, but `exec` never verifies
    /// its value -- only that the flag was set. There is no cryptographic
    /// proof that a given invocation actually came from the hook. Anyone who
    /// can run `runok exec` at all can set it. This is deliberately not
    /// treated as a security hole: it only relaxes `Action::Pass` (a command
    /// that matched no rule), never `Action::Deny` or `Action::Ask`, both of
    /// which `exec` still re-evaluates and rejects regardless of this flag.
    /// So spoofing it can only turn "the caller's own permission flow decides,
    /// unsandboxed" into "the caller's own permission flow decides,
    /// sandboxed" -- a strictly narrower outcome, not a privilege
    /// escalation. CLI flags are not inherited by the spawned command or a
    /// nested `runok exec`.
    hook_origin: bool,
...
impl ExecAdapter {
    ...
    /// Mark this invocation as originating from a hook wrapper.
    pub fn with_hook_origin(mut self, hook_origin: bool) -> Self {
        self.hook_origin = hook_origin;
        self
    }

    /// Allow an `Action::Ask` result to execute after the caller has obtained
    /// its approval through an outer permission flow.
    ...
    /// Resolve one or more sandbox preset names to a single merged
    /// `SandboxPolicy` ("Strictest Wins" via `SandboxPreset::merge_strictest`).
    /// Returns `None` when `preset_names` is empty.
    fn resolve_sandbox_policy(
        ...
        for name in preset_names {
            let preset = self.sandbox_definitions.get(name).ok_or_else(|| {
                anyhow::anyhow!(
    ...
    /// Falls back to the `--sandbox` names passed on the CLI when `sandbox`
    /// carries none (e.g. `defaults.action: allow` with no matching rule).
    fn execute_with_sandbox(&self, sandbox: &SandboxInfo) -> Result<i32, anyhow::Error> {
        ...
        let preset_names: &[String] = if names.is_empty() {
            &self.sandbox_presets
        } else {
            names
        };
        let policy = self.resolve_sandbox_policy(preset_names)?;
...
impl Endpoint for ExecAdapter {
    ...
    fn handle_no_match(&self, defaults: &Defaults) -> Result<i32, anyhow::Error> {
        match defaults.resolved_action() {
            ...
            ActionKind::Ask => {
                if self.ask {
                    let sandbox_names = defaults.sandbox.clone().into_iter().collect::<Vec<_>>();
                    return self.execute_with_sandbox(&SandboxInfo::Preset(sandbox_names));
                }
...
mod tests {
    ...
    fn extract_command_returns_expected(
        ...
        let adapter = ExecAdapter::new(args, vec![], Box::new(MockExecutor::new(0)));
    ...
    // `--hook-origin` only relaxes `Action::Pass` -- a spoofed flag must never
    // let a denied command through. This is the load-bearing guarantee behind
    // treating the flag's forgeability as an accepted risk rather than a
    // security hole (see the doc comment on
    ...
    #[case::exit_0(0)]
    #[case::exit_42(42)]
    fn handle_action_ask_with_flag_executes_and_returns_exit_code(#[case] exit_code: i32) {
        let adapter = ExecAdapter::new(
            vec!["terraform".into(), "apply".into()],
            vec![],
            Box::new(MockExecutor::new(exit_code)),
    ...
    #[rstest]
    fn handle_no_match_ask_with_flag_executes() {
        let adapter = ExecAdapter::new(
            vec!["terraform".into(), "apply".into()],
            vec![],
            Box::new(MockExecutor::new(0)),
    ...
    fn handle_no_match_empty_args_returns_0() {
        let adapter = ExecAdapter::new(vec![], vec![], Box::new(MockExecutor::new(0)));
        let defaults = Defaults {
