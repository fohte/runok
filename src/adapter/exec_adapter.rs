...
pub struct ExecAdapter {
    ...
    /// Claude Code hook adapter (via the `RUNOK_HOOK_ORIGIN` env var), rather
    /// than typed by a user. Only in that case does `Action::Pass` defer to
    /// the sandbox instead of being treated as deny -- the hook already
    /// decided to route the permission decision through Claude Code's own
    /// flow.
    ///
    /// The environment variable behind this (`RUNOK_HOOK_ORIGIN=<token>`)
    /// carries a token that changes on every hook invocation, but `exec`
    /// never verifies the token's value -- only that the env var was set.
    /// There is no cryptographic proof that a given invocation actually came
    /// from the hook. Anyone who can run `runok exec` at all can set it. This
    /// is deliberately not treated as a security hole: it only relaxes
...
mod tests {
    ...
    // `RUNOK_HOOK_ORIGIN` only relaxes `Action::Pass` -- a spoofed env var
    // must never let a denied command through. This is the load-bearing
    // guarantee behind treating the env var's forgeability as an accepted
    // risk rather than a security hole (see the doc comment on
