...
pub struct AuditArgs {
    ...
    /// Re-evaluate each entry's command against the current config and
    /// annotate the output with the result (does not filter)
    #[arg(long)]
    pub recheck: bool,
}
...
mod tests {
    ...
    #[case::audit_default(
        &["runok", "audit"],
        Commands::Audit(AuditArgs { action: None, since: None, until: None, command: None, dir: None, limit: 50, json: false, recheck: false }),
    )]
    #[case::audit_with_action(
        &["runok", "audit", "--action", "deny"],
        Commands::Audit(AuditArgs { action: Some("deny".into()), since: None, until: None, command: None, dir: None, limit: 50, json: false, recheck: false }),
    )]
    #[case::audit_with_since(
        &["runok", "audit", "--since", "1h"],
        Commands::Audit(AuditArgs { action: None, since: Some("1h".into()), until: None, command: None, dir: None, limit: 50, json: false, recheck: false }),
    )]
    #[case::audit_with_dir(
        &["runok", "audit", "--dir", "/home/user/project"],
        Commands::Audit(AuditArgs { action: None, since: None, until: None, command: None, dir: Some("/home/user/project".into()), limit: 50, json: false, recheck: false }),
    )]
    #[case::audit_with_recheck(
        &["runok", "audit", "--recheck"],
        Commands::Audit(AuditArgs { action: None, since: None, until: None, command: None, dir: None, limit: 50, json: false, recheck: true }),
    )]
    #[case::audit_with_all_options(
        &["runok", "audit", "--action", "allow", "--since", "7d", "--until", "1h", "--command", "git", "--limit", "10", "--json", "--recheck"],
        Commands::Audit(AuditArgs { action: Some("allow".into()), since: Some("7d".into()), until: Some("1h".into()), command: Some("git".into()), dir: None, limit: 10, json: true, recheck: true }),
    )]
    #[case::init_defaults(
        &["runok", "init"],
