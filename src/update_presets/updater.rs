...
use crate::config::git_client::{GitClient, RemoteRef};
...
use crate::config::{ConfigError, load_and_resolve_preset_with};

use super::current_timestamp;
use super::semver_utils::{find_upgrade_candidates, parse_version_spec};
...
#[derive(Debug, Clone)]
pub(super) struct SkippedCandidate {
    tag: String,
    source_label: String,
    requirement: String,
    current: String,
...
pub(super) fn update_single_preset<G: GitClient>(
    reference: &str,
    cache: &PresetCache,
    git_client: &G,
...
pub(super) fn warn_skipped_candidates(reference: &str, skipped: &[SkippedCandidate]) {
    if skipped.is_empty() {
        return;
    }
    eprintln!(
