...
fn arb_alternation_pair() -> impl Strategy<Value = (String, Vec<String>)> {
    (
        ...
        .prop_map(|(chosen, mut others)| {
            // Ensure chosen is in the alternation list
...
proptest! {
    ...
    fn prop_matching_pairs_always_allow(
        ...
        // Generate a simple pattern with wildcard and matching command
...
// ========================================
// Property A (extended): literal, alternation, flag patterns
// ========================================
...
proptest! {
    ...
    fn prop_no_panic_evaluate_compound(
        cmds in proptest::collection::vec(
            (arb_cmd_name(), proptest::collection::vec(arb_positional(), 0..=2)),
            1..=3
        ),
        operators in proptest::collection::vec(arb_operator(), 1..=2),
        pattern_cmd in arb_cmd_name(),
        action in prop_oneof![
            Just("allow".to_string()),
            Just("deny".to_string()),
        ...
        let cmd_strings: Vec<String> = cmds.iter()
            .map(|(name, tokens)| build_command(name, tokens))
            .collect();

        // Join with operators (cycling if needed)
        let mut compound = cmd_strings[0].clone();
        for (i, cmd) in cmd_strings.iter().enumerate().skip(1) {
            let op = &operators[i % operators.len()];
            compound = format!("{compound} {op} {cmd}");
        }
...
proptest! {
    ...
    fn prop_rule_order_independence(
        ...
        // Generate 2-4 rules with different actions
        ...
        // Use an index to select a permutation direction (reverse vs original)
        ...
        let mut reversed_lines = rule_lines.clone();
        if reverse {
            reversed_lines.reverse();
        } else {
            // Rotate by 1
            if reversed_lines.len() > 1 {
                let first = reversed_lines.remove(0);
                reversed_lines.push(first);
            }
        }
        let yaml_reordered = format!("rules:\n{}", reversed_lines.join("\n"));

        if let (Ok(config_orig), Ok(config_reord)) = (
            parse_config(&yaml_original),
            parse_config(&yaml_reordered),
        ) {
            let ctx = empty_context();
            let result_orig = evaluate_command(&config_orig, &command, &ctx);
            let result_reord = evaluate_command(&config_reord, &command, &ctx);

            match (result_orig, result_reord) {
                (Ok(r1), Ok(r2)) => {
                    // Compare action variants (ignoring message details)
                ...
                _ => {
                    // If one errors, both should error (or we skip)
                }
            }
        }
    }
